...
contract('CSetter', function (accounts) {
	...
	it('permissions check', async () => {
		...
		await expectRevert(collateral._setSafetyMarginSqrt(SAFETY_MARGIN_TEST, {from: user}), 'Lyf: UNAUTHORIZED');
		await expectRevert(collateral._setLiquidationIncentive(LIQUIDATION_INCENTIVE_TEST, {from: user}), 'Lyf: UNAUTHORIZED');
	});
	...
	it('safety margin boundaries', async () => {
		...
		await expectRevert(collateral._setSafetyMarginSqrt(failMin, {from: admin}), 'Lyf: INVALID_SETTING');
		...
		await expectRevert(collateral._setSafetyMarginSqrt(failMax, {from: admin}), 'Lyf: INVALID_SETTING');
	});
	...
	it('liquidation incentive boundaries', async () => {
		...
		await expectRevert(collateral._setLiquidationIncentive(failMin, {from: admin}), 'Lyf: INVALID_SETTING');
		...
		await expectRevert(collateral._setLiquidationIncentive(failMax, {from: admin}), 'Lyf: INVALID_SETTING');
	});
});