...
contract('BAllowance', function (accounts) {
	...
	beforeEach(async () => {
		token = await BAllowance.new("Lyf", "TAROT");
	});
	...
	it('checkBorrowAllowance fail', async () => {
		...
		await expectRevert(token.checkBorrowAllowance(user, other, TEST_AMOUNT), "Lyf: BORROW_NOT_ALLOWED");
		await token.borrowApprove(other, TEST_AMOUNT, {from: user});
		await expectRevert(token.checkBorrowAllowance(user, other, TEST_AMOUNT.add(new BN(1))), "Lyf: BORROW_NOT_ALLOWED");
