...
contract('Collateral', function (accounts) {
	...
	describe('getPrices', () => {
		...
		before(async () => {
			...
			await collateral.setPriceOracle(factory.obj.lyfPriceOracle.address);
		...
		[
			...
			it(`getPrices for ${JSON.stringify(testCase)}`, async () => {
				...
				await factory.obj.lyfPriceOracle.setPrice(underlying.address, uq112(priceOracle));
		...
		it(`fail if price(0|1) <= 100`, async () => {
			...
			await factory.obj.lyfPriceOracle.setPrice(underlying.address, priceUQ112);
